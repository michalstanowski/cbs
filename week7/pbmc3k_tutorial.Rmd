...
pbmc.data <- Read10X(data.dir = "C:\\Users\\Michał\\Documents\\Studia\\StudiaMagisterskie\\Bioinformatyka\\RokISemII\\CBS\\scRNA1\\pbmc3k_filtered_gene_bc_matrices\\filtered_gene_bc_matrices\\hg19")

pbmc <- CreateSeuratObject(counts = pbmc.data, project = "pbmc3k", min.cells = 3, min.features = 200) #contains both data and analysis
pbmc
```
...
Identifying the true dimensionality of a dataset -- can be challenging/uncertain for the user. We therefore suggest these three approaches to consider. The first is more supervised, exploring PCs to determine relevant sources of heterogeneity, and could be used in conjunction with GSEA (Gene set enrichment analysis) for example. The second implements a statistical test based on a random null model, but is time-consuming for large datasets, and may not return a clear PC cutoff. The third is a heuristic that is commonly used, and can be calculated instantly. In this example, all three approaches yielded similar results, but we might have been justified in choosing anything between PC 7-12 as a cutoff. 
...
pbmc.markers %>% group_by(cluster) %>% top_n(n = 20, wt = avg_log2FC) -> top10
...
```{r hw1}
# the data was downloaded through wget command

# loading the data
pbmc68.data <- Read10X(data.dir = "C:\\Users\\Michał\\Documents\\Studia\\StudiaMagisterskie\\Bioinformatyka\\RokISemII\\CBS\\scRNA1\\filtered_matrices_mex\\hg19")
pbmc68 <- CreateSeuratObject(counts = pbmc68.data, project = "pbmc3k", min.cells = 3, min.features = 200) # contains both data and analysis
pbmc68
...
```{r qc}
# QC
VlnPlot(pbmc68, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3) 
plot1 <- FeatureScatter(pbmc68, feature1 = "nCount_RNA", feature2 = "percent.mt") 
plot2 <- FeatureScatter(pbmc68, feature1 = "nCount_RNA", feature2 = "nFeature_RNA") 
plot1 + plot2
pbmc68 <- subset(pbmc68, subset = nFeature_RNA > 200 & nFeature_RNA < 2000 & percent.mt < 12)

# normalizing the data
# med_umi <- median(pbmc68@meta.data$nCount_RNA) #Zheng used median as a scaling_factor
# pbmc68 <- NormalizeData(pbmc68, scale.factor = med_umi)
pbmc68 <- NormalizeData(pbmc68)

# feature selection
pbmc68 <- FindVariableFeatures(pbmc68, selection.method = "vst", nfeatures = 2000) # PCA on 1000 most variable genes in Zheng, we do it on 2000
top10 <- head(VariableFeatures(pbmc68), 10)
plot1 <- VariableFeaturePlot(pbmc68)
plot2 <- LabelPoints(plot = plot1, points = top10, repel = TRUE)
plot1 + plot2

# scaling the data
all.genes <- rownames(pbmc68)
pbmc68 <- ScaleData(pbmc68, features = all.genes)
```

```{r pca}
# PCA
pbmc68 <- RunPCA(pbmc68, features = VariableFeatures(object = pbmc68))
# pbmc68[['pca']]
print(pbmc68[["pca"]], dims = 1:5, nfeatures = 5)
VizDimLoadings(pbmc68, dims = 1:2, reduction = "pca")
DimPlot(pbmc68, reduction = "pca") + NoLegend()
DimHeatmap(pbmc68, dims = 1, cells = 500, balanced = TRUE)
DimHeatmap(pbmc68, dims = 1:15, cells = 500, balanced = TRUE)

# dimensionality
pbmc68 <- JackStraw(pbmc68, num.replicate = 100)
pbmc68 <- ScoreJackStraw(pbmc68, dims = 1:15) #thresolding
JackStrawPlot(pbmc68, dims = 1:15) # 22 significant PCs in Zheng

ElbowPlot(pbmc68) # let's choose 15
```

```{r clustering}
# clustering
pbmc68 <- FindNeighbors(pbmc68, dims = 1:15) # 22 significant PCs in Zheng
pbmc68 <- FindClusters(pbmc68, resolution = 0.12) # resolution 0.11 gave 10 clusters, just like in Zheng (unfortunately "optimal resoultion increases with larger datasets", so this is probably not the best resolution)
head(Idents(pbmc68), 5)
```

```{r tsne}
# tsne
pbmc68 <- RunTSNE(pbmc68, dims = 1:15) # 22 in Zheng
DimPlot(pbmc68, reduction = "tsne")
```

#finding markers
```{r markers}
#cluster2.markers <- FindMarkers(pbmc68, ident.1 = 2)
#head(cluster2.markers, n = 5)
#cluster5.markers <- FindMarkers(pbmc68, ident.1 = 5, ident.2 = c(0, 3))
#head(cluster5.markers, n = 5)

pbmc68.markers <- FindAllMarkers(pbmc68, # had to speed up for the large dataset, changed the parameters proposed by Seurat
                          only.pos = TRUE,
                          min.pct = 0.25,
...
VlnPlot(pbmc68, features = c("MS4A1", "CD79A"))
VlnPlot(pbmc68, features = c("NKG7", "PF4"), slot = "counts", log = TRUE)
FeaturePlot(pbmc68, features = c("MS4A1", "GNLY", "CD3E", "CD14", "FCER1A", "FCGR3A", "LYZ", "PPBP",
    "CD8A"))
...
DoHeatmap(pbmc68, features = top10$gene) + NoLegend()


# recreating Zheng, 2017
cluster_counts <- table(Idents(pbmc68))
cluster_percents <- round(100 * cluster_counts / sum(cluster_counts), 1)

new_labels <- paste0("Cluster ", names(cluster_counts), " (", cluster_percents, "%)")

names(new_labels) <- levels(Idents(pbmc68))
pbmc68 <- RenameIdents(pbmc68, new_labels)

pdf("stanowski_fig1.pdf", width = 8, height = 6)

DimPlot(pbmc68, reduction = "tsne", label = TRUE, pt.size = 0.5) + NoLegend()

dev.off()

new_labels <- c("Naive CD4+", "CD8+", "Activated CD8+", "Memory and Reg T", "Monocytes and dendritic", "B", "B, dendritic, T", "Megakaryocytes", "NK", "Naive CD8+") # based on Zheng, 2017  (Fig. 3c) and pbmc68$markers analysis

names(new_labels) <- levels(pbmc68)
pbmc68 <- RenameIdents(pbmc68, new_labels)


pdf("stanowski_fig1_annotated.pdf", width = 8, height = 6)
DimPlot(pbmc68, reduction = "tsne", label = TRUE, pt.size = 0.5) + NoLegend()
dev.off()

# the markers are kinda correct, but there was possibly mislabeling of labels between megakaryoctes and NK cells, probably because of loosening the parameters in FindAllMarkers function
```
...
library(cluster)
library(factoextra)
# overall t-SNE visualization with all clusters -- make sure that hierarchy (group) is visualized.  - done above
pca_mat <- Embeddings(pbmc68, "pca")[, 1:10]
tsne_mat <- Embeddings(pbmc68, "tsne")
clusters <- Idents(pbmc68)
...
for (cl in levels(clusters)) {
  ...
  pca_sub <- pca_mat[cells, ]
  tsne_sub <- tsne_mat[cells, ]
  
  
  sil_scores <- numeric()

  for (k in 2:4) {
    km.res <- kmeans(pca_sub, centers = k, nstart = 25)
    ss <- silhouette(km.res$cluster, dist(pca_sub))
  ...
  cat("Optimal K =", optimal_k, "\n")
    ...
    km <- kmeans(pca_sub, centers = optimal_k)
  ...
  tsne_plots[[as.character(cl)]] <- ggplot(df, aes(x = tSNE_1, y = tSNE_2, color = kmeans)) +
    geom_point(size = 1) +
    labs(title = paste("Cluster", cl, "- k-means (K =", optimal_k, ")")) +
    theme_minimal() +
...
pdf("tsne_subclusters.pdf", width = 8, height = 6)
