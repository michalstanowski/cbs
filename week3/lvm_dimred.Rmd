...
pdf("stanowski_problem1.pdf",height=10,width=10)
heatmap.2(edata,
          main = "Heatmap Problem 1", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
          trace="none",         # turns off trace lines inside the heat map
          margins =c(12,9),     # widens margins around plot
...
In order to properly plot the points I used the information from this SOF thread:
https://stackoverflow.com/questions/69622025/aes-mapping-in-ggplot-of-a-pasted-name


```{r plots}
for (i in 1:5) { # 5 limit is based on the scree plot (elbow)
  for (j in 1:i) {
      print(ggplot(PC) +
        geom_point(aes(x = .data[[paste0("V", i)]], y = .data[[paste0("V", j)]], col = as.factor(strain))))
  }
}

# v2 and v3 
```
Let's choose V2 and V3 and save this plot.

```{r save}
pdf("stanowski_problem2.pdf",height=10,width=10)

ggplot(PC) + geom_point(aes(x = V2, y = V3, col = as.factor(strain))) + labs(
  title = "Strain separation by PC2 and PC3",
  x = "PC2",
  y = "PC3",
  color = "Strain"
)
...
```{r hw3}
pdf("stanowski_problem3.pdf",height=10,width=10)

plot(svd.out$u[,1], svd.out$u[,2],pch=20,
     main = "Scatter plot of the top 2 left singular vectors",
     xlab = "1st left singular vector",
     ylab = "2nd left singular vector")
...
pdf("stanowski_problem4.pdf",height=10,width=10)

top5 <- as.data.frame(svd.out$u[, 1:5])

top5.tidy <- gather(top5, key="Vector", value="Loading_value")

ggplot(top5.tidy, aes(x=Vector,y=Loading_value)) + geom_violin() + labs(
  title="Violin plots of the top 5 left singular vectors", x ="Vector", y="Loading value"
)
...
# k-means clustering
km_clustered <- kmeans(edata, centers=5)

# t-sne
tsne_out <- Rtsne(edata, pca=FALSE,perplexity=60)
tsne_out <- data.table(tsne_out$Y)

plot5 <- ggplot(tsne_out) + geom_point(aes(x=V1, y=V2, col=as.factor(km_clustered$cluster))) + labs(title="Scatter plot of t-SNE projection of genes clustered using K-means", col="K-means clusters") # coloring using the k-means clusters

plot5

pdf("stanowski_problem5.pdf",height=10,width=10)
